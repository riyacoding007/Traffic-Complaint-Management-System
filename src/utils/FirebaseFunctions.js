...
export const createComplaint = async (formData, media) => {
  ...
  try {
    let updatedFormData = { ...formData, timestamp };

    if (media) {
      const ext = media.name ? media.name.split(".").pop() : "";
      const fileName = `complaints/${timestamp}${ext ? `.${ext}` : ""}`;
      const fileRef = ref(storage, fileName);
      await uploadBytes(fileRef, media);
      const fileLink = await getDownloadURL(fileRef);
      updatedFormData = { ...updatedFormData, mediaPath: fileLink };
    } else {
      updatedFormData = { ...updatedFormData, mediaPath: "" };
    }
  ...
  return onSnapshot(complaintsCollection, async (complaintsSnapshot) => {
    ...
    for (const complaintDoc of complaintsSnapshot.docs) {
      ...
      const complaintWithAuthor = {
        id: complaintId,
        author: userData && userData.name ? userData.name : "Unknown",
        ...complaintData,
        comments: [],
      };
